...
function digital_newspaper_widget_fields( $instance, $args, $field_value ) {
    echo '<div class="digital-newspaper-widget-field digital-newspaper-' .esc_html( $args['type'] ). '-field">';
        switch( $args['type'] ) {
            case 'number' : ?>
            ...
            case 'icon-text' : $field_value_formatted = json_decode( $field_value );
                               $icons = digital_newspaper_get_tabbed_icon_classes();
             ?>
                        <h2 class="title"><?php echo esc_html( $args['title'] ); ?></h2>
                        <?php
                            if( isset( $args['description'] ) ) {
...
function digital_newspaper_sanitize_widget_fields( $widget_field, $new_instance ) {
